...
public class ImageHiding extends JFrame implements ActionListener
{
 ...
 public void actionPerformed(ActionEvent event)
 {
  ...
  if (source == encodeBitsPlus)
  {
   ...
   maxchar.setText(Integer.toString((bits*getHostImage().getHeight()*getHostImage().getWidth()*3)/8));
   //encodecharval.setText();
 ...
 public ImageHiding()
 {
  ...
  this.add(new JLabel("Maxium number of characters that can be embedded:"));
  maxchar=new JTextField("0",5);
  maxchar.setEditable(false);
  	
  gbc.weightx = -1.0;
  layout.setConstraints(maxchar, gbc);
  this.add(maxchar);
...
class Steganography
{
 ...
 public void encode(int encodeBits)
 {
  ...
  for (int i = 0; i < imageRGB.length; i++)
  {
	  //if(textbits.length()-4*encodeBits > start) {
	  int m=0;
	  for(int n=start;n<start+4*encodeBits;n=n+encodeBits)
	  {
			 String x=textbits.substring(n,n+encodeBits);
			 //System.out.println("x:"+x);
			  data[m]=Integer.parseInt(x,2);	  
			  m++;
			  count = n+encodeBits;
	  }
	  start=count;
  ...
  // int encodeData = (encodeRGB[i] & encodeMask) >>> (8 - encodeBits);
	  h=data[0] << 24 | data[1] << 16 | data[2] << 8 | data[3];
	  
	  //System.out.println("h:"+h);
	 
   imageRGB[i] = (imageRGB[i] & hostMask) | (h & ~hostMask);
		  }
	 ...
	 try{
	  ...
	  try {
		...
		while( true ) {
			...
			String no=Integer.toBinaryString((input.readChar()));
			//System.out.println(no);
	        sb.append(String.format("%8s", no).replace(' ','0'));
